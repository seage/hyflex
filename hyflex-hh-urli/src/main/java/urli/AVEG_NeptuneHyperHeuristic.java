 ...
 */
public class AVEG_NeptuneHyperHeuristic extends HyperHeuristic {

	/**
	 * Constructor. Accepts a seed to initialize a Pseudo-Random Number Generator.
	 * @param seed seed for the PSRNG.
	 */
	public AVEG_NeptuneHyperHeuristic(long seed) {
		super(seed);
		rng.setSeed(seed);
	}
	...
	Map<Pair<HeuristicType,Double>, Double> actions = new HashMap<Pair<HeuristicType,Double>, Double>();
	...
	Map<Integer, Map<Integer,Map<Pair<HeuristicType,Double>, Double>>> actionValues = new HashMap<Integer, Map<Integer, Map<Pair<HeuristicType,Double>,Double>>>();
	...
	protected void solve(ProblemDomain problemDomain) {
		...
		for(int h = 0; h < heuristicTypes.length; h++)
			if ( problemDomain.getHeuristicsOfType(heuristicTypes[h]) != null )
				for(int q = 0; q < quantities.length; q++)
					actions.put(new Pair<HeuristicType,Double>(heuristicTypes[h], quantities[q]),  0.0);
		...
		for( int agent = 0; agent < agents; agent++) {
			...
			actionValues.put(agent, new HashMap<Integer, Map<Pair<HeuristicType,Double>,Double>>());
		}
		...
		while(!hasTimeExpired()) {
			...
			for(int agent = 0; agent < agents; agent++) {
				...
				Pair<HeuristicType,Double> action = chooseAction(agent);
				double reward = functionValue[agent];

				int[] heuristics = problemDomain.getHeuristicsOfType(action.getKey());
				int heuristicIndex = heuristics[rng.nextInt(heuristics.length)];
					
				// Setup low-level heuristic
				problemDomain.setDepthOfSearch(action.getValue());
				problemDomain.setIntensityOfMutation(action.getValue());
	...
	private Pair<HeuristicType,Double> chooseAction(int agent) {
		
		// With probability epsilon choose randomly
		if ( rng.nextDouble() <= epsilon )
			return (Pair<HeuristicType,Double>) actions.keySet().toArray()[rng.nextInt(actions.size())];
	...
	private void initializeActionValues(int agent, int state) {
		...
		actionValues.get(agent).put(state, new HashMap<Pair<HeuristicType,Double>, Double>());
		Iterator<Pair<HeuristicType,Double>> actionIterator = actions.keySet().iterator();
	 ...
	 * @param agent index of itself.
	 */
	private Pair<HeuristicType,Double> chooseActionGreedily(int agent) {
		
		Map<Pair<HeuristicType,Double>, Double> currentActionValues = actionValues.get(agent).get(state[agent]);
		Iterator<Pair<HeuristicType,Double>> actionValueIterator = currentActionValues.keySet().iterator();
		
		Pair<HeuristicType,Double> bestAction = actionValueIterator.next();
		
		// Choose best
		while (actionValueIterator.hasNext()) {
			Pair<HeuristicType,Double> action = actionValueIterator.next();
