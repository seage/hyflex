...
public class SelectBestDepth<T extends BasicSolution<P>, P extends BenchmarkInfo, N extends NeighbourHood<P> & RandomIterable> extends Selector<T, P, N> {
  EvaluationFunction<T, P> evalf;
  
  boolean strict;
  
  public SelectBestDepth(boolean strict) {
    this.strict = strict;
    this.evalf = (EvaluationFunction<T, P>)new ObjectiveFunction();
  }
  
  public SelectBestDepth(boolean strict, EvaluationFunction<T, P> evalf) {
    this.strict = strict;
    this.evalf = evalf;
  }
  ...
  public Proposal<T, P> select(T c, Modifier<T, P, N> modifier, int max) {
    ...
    if (modifier.isApplicable((BasicSolution)c)) {
      int dos = modifier.interpretDOS(Math.min(this.params.getDOS(this), max), (BasicSolution)c);
  ...
  private Result searchBest(T c, Modifier<T, P, N> modifier, int dos) {
    ...
    IteratorNH it = ((RandomIterable)neighbourHood).getIterator();
...
/* Location:              C:\Users\Steve\Documents\GitHub\hyflext\domains\hyflex_ext.jar!\hfu\heuristics\selector\SelectBestDepth.class
