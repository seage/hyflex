...
public class SelectFirstDepth<T extends BasicSolution<P>, P extends BenchmarkInfo, N extends NeighbourHood<P> & RandomIterable> extends Selector<T, P, N> {
  ...
  public Proposal<T, P> select(T c, Modifier<T, P, N> modifier, int max) {
    ...
    if (modifier.isApplicable((BasicSolution)c)) {
      int dos = Math.min(modifier.interpretDOS(this.params.getDOS(this), (BasicSolution)c), max);
...
/* Location:              C:\Users\Steve\Documents\GitHub\hyflext\domains\hyflex_ext.jar!\hfu\heuristics\selector\SelectFirstDepth.class
